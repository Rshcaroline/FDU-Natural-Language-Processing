...
def TextFeatures(text, k):
    ...
    for title in [2*i for i in range(0,int(n))]:   # [0,2,4,6,...]
        pos_word, neg_word, not_word, degree_word = SentiScore.\
            LocateSpecialWord(pos_dict, neg_dict, not_dict, degree_dict, sorted(set(text[title]),key=text[title].index))
        title_score.append(SentiScore.ScoreSent(pos_word, neg_word, not_word, degree_word, sorted(set(text[title]),key=text[title].index)))
        pos = len(pos_word) * k
        neg = len(neg_word) * k
    features['title_score'] = round(np.mean(title_score)) * k

    content_score = []
    for content in [2*i+1 for i in range(0,int(n))]:   # [1,3,5,7,...]
        pos_word, neg_word, not_word, degree_word = SentiScore.\
            LocateSpecialWord(pos_dict, neg_dict, not_dict, degree_dict, sorted(set(text[title]),key=text[title].index))
        content_score.append(SentiScore.ScoreSent(pos_word, neg_word, not_word, degree_word, sorted(set(text[title]),key=text[title].index)))
        pos += len(pos_word)
        neg += len(neg_word) + len(not_dict)
    features['content_score'] = round(np.mean(content_score))

    features['pos_word'] = round(pos/(10*n))
    features['neg_word'] = round(neg/(10*n))
...
if __name__ == '__main__':
    ...
    train_set, test_set = PrepareSets(train_group[1000:], train_group[:1000],5)
    stop = time.time()
